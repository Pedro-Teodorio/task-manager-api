...
@Tag(name = "Tarefas V2", description = "Gerenciamento de tarefas na versão 2 da API")
@RateLimit(
        value = 10, // Limite de 10 requisições por segundo
        window = 1, // Janela de 1 segundo
        windowUnit = ChronoUnit.MINUTES
)
public class TarefaResourceV2 {
    ...
    @POST
    @Transactional
    @Operation(summary = "Cria uma nova tarefa (idempotente)", description = "Cadastra uma nova tarefa vinculada a um projeto e, opcionalmente, atribui usuários.")
    ...
    @Path("{id}")
    @Transactional
    @Operation(summary = "Atualiza uma tarefa (idempotente)", description = "Atualiza os dados de uma tarefa existente.")
