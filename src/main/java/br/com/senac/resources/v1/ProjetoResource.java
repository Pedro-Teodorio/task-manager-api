...
@Tag(name = "Projetos", description = "Gerenciamento de projetos do sistema")
public class ProjetoResource {
    ...
    @Path("/usuario/{userId}")
    @Transactional
    @Operation(summary = "Cria um novo projeto para um usuário", description = "Cadastra um novo projeto vinculado a um usuário.")
    @APIResponse(responseCode = "201", description = "Projeto criado com sucesso",
            content = @Content(schema = @Schema(implementation = ProjetoDTO.class)))
    @APIResponse(responseCode = "404", description = "Usuário não encontrado")
    public Response createForUser(@PathParam("userId") Long userId,
                                  ProjetoDTO dto,
        ...
        URI uri = uriInfo.getAbsolutePathBuilder()
                .path(Long.toString(p.id)).build();
        return Response.created(uri).entity(ProjetoDTO.fromEntity(p)).build();
    }
